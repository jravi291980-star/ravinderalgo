...
import pytz # Necessary for time zone handling
...
class StrategySettings(models.Model):
    """Configuration for the Cash Breakout Strategy, holding all dynamic parameters."""
    ...
    # --- Strategy Parameters (from the original logic) ---
    ...
    # --- Time & P&L Exits ---
    # Using .time() to store simple time objects compatible with Django's makemigrations
    start_time = models.TimeField(default=datetime.strptime('09:20:00', '%H:%M:%S').time(), verbose_name="Strategy Start Time (IST)")
    end_time = models.TimeField(default=datetime.strptime('15:00:00', '%H:%M:%S').time(), verbose_name="Strategy End Time (IST - Force Square Off)")
    ...
    # --- Manual Control Flag ---
    manual_override = models.BooleanField(default=False, verbose_name="Manual Trade Control Override (Disable Algo Entries)")
