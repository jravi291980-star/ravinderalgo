...
# --- NIFTY 500 Symbol List (Truncated for readability, full list used internally) ---
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not token:
                raise CommandError("Dhan Access Token not found in Redis. Generate token via dashboard first.")
        ...
        try:
            # We use "full" mode to ensure all necessary metadata (like exchangeSegment) is available.
        ...
        instrument_map = {}
        missing_symbols = set(NIFTY_500_SYMBOLS)
        total_instruments = 0
        ...
        for item in response['data']:
            ...
            if symbol and security_id and exchange_segment and symbol in missing_symbols:
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Successfully cached {total_instruments} instruments to Redis key: {settings.SYMBOL_ID_MAP_KEY}"))

            if missing_symbols:
                self.stdout.write(self.style.WARNING(
                    f"{len(missing_symbols)} symbols were not found or matched in the API response (Expected for large lists)."))
